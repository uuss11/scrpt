...
local function validateGeneratedScript(scriptText)
    ...
    local hasLegacyMain = string.find(scriptText, "local function main()", 1, true) ~= nil and string.match(finalText, "main%(%)[%s]*$") ~= nil
    local hasModernCallback = string.find(scriptText, "RunService.RenderStepped:Connect(function()", 1, true) ~= nil and string.match(finalText, "end%)%s*$") ~= nil
    local emptyModernCallback = string.match(finalText, "RunService%.RenderStepped:Connect%(%s*function%(%s*%)%s*end%)%s*$") ~= nil
    if GENERATED_SCRIPT_SHAPE == "main" then
        ...
        if not string.find(scriptText, "task.delay(10, printSummary)", 1, true) then
...
-- RE V13 staged adapter: appended after the untouched V8 working script.
...
local v13BootStage = "V8 baseline loaded"
...
pcall(function() v13BootDiagnostic(v13BootStage, "Stage0", "V8 baseline", "PASS", "", "append V13 adapter") end)
...
-- V13 visual pass: deferred and isolated; the V8 GUI remains the fallback.
task.defer(function()
    pcall(function()
                ...
                for _, child in ipairs(descendants) do
                    pcall(function()
                        if child:IsA("GuiObject") then
                            ...
                            if opaque and child.BackgroundTransparency > 0 and child ~= title and child ~= inspectorTitle and child ~= modelTitle then
                                child.BackgroundTransparency = math.min(child.BackgroundTransparency, 0.04)
                            end
                        end
                    end)
                end
        ...
        if menuFrame then menuFrame.BackgroundColor3 = Color3.fromRGB(18, 24, 38) end
        if inspectorFrame then inspectorFrame.BackgroundColor3 = Color3.fromRGB(12, 20, 34) end
        if aiFrame then aiFrame.BackgroundColor3 = Color3.fromRGB(14, 22, 38) end
        if modelFrame then modelFrame.BackgroundColor3 = Color3.fromRGB(18, 25, 43) end
        local function v13HideInspectorChildren()
            if aiFrame then aiFrame.Visible = false end
            if modelFrame then modelFrame.Visible = false end
            if rightScroll then rightScroll.Visible = true end
        end
        rememberV12Connection(iconBtn.MouseButton1Click:Connect(function()
            if menuFrame and menuFrame.Visible then
                if inspectorFrame then inspectorFrame.Visible = false end
                v13HideInspectorChildren()
            end
        end))
        rememberV12Connection(closeBtn.MouseButton1Click:Connect(function()
            if menuFrame then menuFrame.Visible = false end
        end))
        rememberV12Connection(toggleInspectorBtn.MouseButton1Click:Connect(function()
            if inspectorFrame and inspectorFrame.Visible then
                if menuFrame then menuFrame.Visible = false end
                v13HideInspectorChildren()
            else
                v13HideInspectorChildren()
        ...
        rememberV12Connection(inspectorCloseBtn.MouseButton1Click:Connect(function()
            if inspectorFrame then inspectorFrame.Visible = false end
            v13HideInspectorChildren()
        end))
        rememberV12Connection(aiTabBtn.MouseButton1Click:Connect(function()
            if menuFrame then menuFrame.Visible = false end
            if inspectorFrame then inspectorFrame.Visible = true end
            if modelFrame then modelFrame.Visible = false end
            if rightScroll then rightScroll.Visible = false end
            if aiFrame then aiFrame.Visible = true end
        end))
        rememberV12Connection(modelTabBtn.MouseButton1Click:Connect(function()
            if menuFrame then menuFrame.Visible = false end
            if inspectorFrame then inspectorFrame.Visible = true end
            if modelFrame and modelFrame.Visible then
                if aiFrame then aiFrame.Visible = false end
                if rightScroll then rightScroll.Visible = false end
            else
                if rightScroll then rightScroll.Visible = true end
        ...
        rememberV12Connection(modelCloseBtn.MouseButton1Click:Connect(function()
            if modelFrame then modelFrame.Visible = false end
            if rightScroll then rightScroll.Visible = true end
        end))
    end)
...
v13Panel.Parent = inspectorFrame
    ...
    v13Panel.BackgroundColor3 = Color3.fromRGB(12, 20, 34)

...
-- RE V13 staged adapter: appended after the untouched V11 working recorder script.
...
-- V13 visual pass: deferred and isolated; V11 owns all legacy event handlers and recorder lifecycle.
task.defer(function()
    pcall(function()
        ...
        -- Button ownership is intentionally not duplicated here.
        -- V11 owns icon/menu/inspector/AI/model/recorder handlers, while the V13 suffix owns only its own tab controls.
    end)
end)

...
local inspectorScale = nil
local leftScroll = nil
local rightScroll = nil
local aiFrame = nil
local modelFrame = nil
local menuScale = nil
local v13StartLegacyMonitor = nil
local function refreshRuntimeReferences()
    if not LocalPlayer then
        local okPlayer, player = pcall(function() return Players.LocalPlayer end)
        if okPlayer and player then LocalPlayer = player end
    end
    if not Camera then
        local okCamera, currentCamera = pcall(function() return Workspace.CurrentCamera end)
        if okCamera and currentCamera then Camera = currentCamera end
    end
    if LocalPlayer and not Mouse then pcall(function() Mouse = LocalPlayer:GetMouse() end) end
end
...
task.spawn(function()
    for attempt = 1, 40 do
        ...
        refreshRuntimeReferences()
        if LocalPlayer and Camera then break end
        task.wait(0.25)
    end
end)
...
local v13BootstrapGui = nil
local v13BootstrapIcon = nil
local v13BootstrapRecovery = nil
local v13BootstrapHost = nil

local function v13BootstrapPrint(message)
    pcall(function() print("[RE v13 BOOT] " .. tostring(message)) end)
end

local function v13BootstrapRefreshPlayer()
...
local function v13BootstrapPlayerGui()
    if not v13BootstrapRefreshPlayer() or not LocalPlayer then return nil end
    local ok, playerGui = pcall(function()
        local existing = LocalPlayer:FindFirstChildOfClass("PlayerGui")
        if existing then return existing end
        return LocalPlayer:WaitForChild("PlayerGui", 6)
    end)
    if ok and playerGui then return playerGui end
    return nil
end

local function v13BootstrapGetHui()
    local gethuiFunction = nil
    local okLookup = pcall(function() gethuiFunction = gethui end)
    if okLookup and type(gethuiFunction) == "function" then
        local okHost, host = pcall(gethuiFunction)
        if okHost and host then return host end
    end
    return nil
end

local function v13BootstrapDestroyOwned(parent)
    if not parent then return end
    pcall(function()
        local old = parent:FindFirstChild("RE_Ultimate_Framework")
        if old then old:Destroy() end
    end)
end

local function v13BootstrapTryHost(screenGui, host)
    if not screenGui or not host then return false end
    local okParent = pcall(function()
        screenGui.Parent = nil
        screenGui.Parent = host
    end)
    if okParent and screenGui.Parent == host then
        v13BootstrapHost = host
        v13BootstrapPrint("GUI HOST: " .. tostring(host))
        return true
    end
    v13BootstrapPrint("GUI HOST FAILED: " .. tostring(host))
    return false
end

local function v13BootstrapAttach(screenGui)
    if not screenGui then return false end
    local hui = v13BootstrapGetHui()
    if hui and v13BootstrapTryHost(screenGui, hui) then return true end
    if CoreGui and v13BootstrapTryHost(screenGui, CoreGui) then return true end
    local playerGui = v13BootstrapPlayerGui()
    if playerGui and v13BootstrapTryHost(screenGui, playerGui) then return true end
    v13BootstrapHost = nil
    return false
end

local function v13BootstrapBuild()
    local ok, err = pcall(function()
        local initialHui = v13BootstrapGetHui()
        local playerGui = nil
        if not initialHui and not CoreGui then playerGui = v13BootstrapPlayerGui() end
        v13BootstrapDestroyOwned(initialHui)
        v13BootstrapDestroyOwned(CoreGui)
        v13BootstrapDestroyOwned(playerGui)

        v13BootstrapGui = Instance.new("ScreenGui")
        v13BootstrapGui.Name = "RE_Ultimate_Framework"
        v13BootstrapGui.ResetOnSpawn = false
        v13BootstrapGui.IgnoreGuiInset = true
        v13BootstrapGui.DisplayOrder = 999

        if not v13BootstrapAttach(v13BootstrapGui) then
            v13BootstrapPrint("GUI HOST UNAVAILABLE")
            return false
        end

        v13BootstrapIcon = Instance.new("TextButton")
        v13BootstrapIcon.Name = "RE_BootIcon"
        v13BootstrapIcon.Size = UDim2.new(0, 40, 0, 40)
        v13BootstrapIcon.Position = UDim2.new(0, 15, 0, 15)
        v13BootstrapIcon.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
        v13BootstrapIcon.BorderSizePixel = 2
        v13BootstrapIcon.BorderColor3 = Color3.fromRGB(255, 50, 50)
        v13BootstrapIcon.Text = "RE"
        v13BootstrapIcon.TextColor3 = Color3.fromRGB(255, 50, 50)
        v13BootstrapIcon.Font = Enum.Font.Code
        v13BootstrapIcon.TextSize = 20
        v13BootstrapIcon.Visible = true
        v13BootstrapIcon.Active = true
        v13BootstrapIcon.Selectable = true
        v13BootstrapIcon.ZIndex = 1000
        v13BootstrapIcon.Parent = v13BootstrapGui
        if v13BootstrapIcon.Parent ~= v13BootstrapGui then
            error("RE_BootIcon parent verification failed")
        end

        v13BootstrapRecovery = Instance.new("Frame")
        v13BootstrapRecovery.Name = "RE_BootRecovery"
        v13BootstrapRecovery.Size = UDim2.new(0, 280, 0, 92)
        v13BootstrapRecovery.Position = UDim2.new(0, 65, 0, 15)
        v13BootstrapRecovery.BackgroundColor3 = Color3.fromRGB(18, 22, 32)
        v13BootstrapRecovery.BorderSizePixel = 1
        v13BootstrapRecovery.BorderColor3 = Color3.fromRGB(255, 150, 70)
        v13BootstrapRecovery.Visible = false
        v13BootstrapRecovery.ZIndex = 999
        v13BootstrapRecovery.Parent = v13BootstrapGui

        local recoveryText = Instance.new("TextLabel")
        recoveryText.Size = UDim2.new(1, -16, 1, -16)
        recoveryText.Position = UDim2.new(0, 8, 0, 8)
        recoveryText.BackgroundTransparency = 1
        recoveryText.Text = "RE ULTIMATE CONSOLE v13\nBOOT RECOVERY\nMain UI is still initializing..."
        recoveryText.TextColor3 = Color3.fromRGB(240, 240, 240)
        recoveryText.Font = Enum.Font.Code
        recoveryText.TextSize = 12
        recoveryText.TextWrapped = true
        recoveryText.TextXAlignment = Enum.TextXAlignment.Left
        recoveryText.Parent = v13BootstrapRecovery

        v13BootstrapIcon.MouseButton1Click:Connect(function()
            if v13BootstrapRecovery then
                v13BootstrapRecovery.Visible = not v13BootstrapRecovery.Visible
            end
        end)
        v13BootstrapPrint("ScreenGui: OK")
        v13BootstrapPrint("ICON: OK")
        return true
    end)
    if not ok then
        v13BootstrapPrint("BOOT ERROR: " .. tostring(err))
        return false
    end
    return err == true or (v13BootstrapGui and v13BootstrapGui.Parent ~= nil and v13BootstrapIcon ~= nil)
end
...
local RE_DESIGN = {
    Colors = {
        ...
        Glass = Color3.fromRGB(20, 29, 46),
        GlassDeep = Color3.fromRGB(13, 20, 34),
        Card = Color3.fromRGB(24, 38, 58),
        MicroGlass = Color3.fromRGB(32, 48, 72),
        Floating = Color3.fromRGB(27, 39, 66),
        Primary = Color3.fromRGB(74, 176, 235),
        Secondary = Color3.fromRGB(132, 101, 235),
        Accent = Color3.fromRGB(91, 224, 222),
        Border = Color3.fromRGB(112, 161, 205),
        Text = Color3.fromRGB(236, 244, 255),
        Muted = Color3.fromRGB(151, 170, 198),
        Success = Color3.fromRGB(78, 196, 145),
        Warning = Color3.fromRGB(232, 178, 88),
        Danger = Color3.fromRGB(220, 103, 118),
        Info = Color3.fromRGB(99, 159, 232)
    },
...
local AI_REQUEST_TIMEOUT_SECONDS = nil
local v13BootStage = "Script start"
...
local function v13BootDiagnostic(stage, moduleName, functionName, status, errorText, recovery)
    ...
    pcall(function()
        print(string.format("RE ULTIMATE CONSOLE v13 | BOOT DIAGNOSTIC | Stage: %s | Module: %s | Function: %s | Status: %s | Error: %s | Recovery: %s", item.Stage, item.Module, item.Function, item.Status, item.Error, item.Recovery))
    end)
    return item
end
...
local function restoreV12State()
    for part, state in pairs(v12OriginalPartState) do
        pcall(function()
    ...
    for humanoid, state in pairs(v12OriginalHumanoidState) do
        pcall(function()
            if humanoid and humanoid.Parent then
                humanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, state.DeadEnabled)
                humanoid.MaxHealth = state.MaxHealth
                humanoid.Health = math.min(state.Health, state.MaxHealth)
            end
        end)
    end
    v12OriginalPartState = {}
    v12OriginalHumanoidState = {}
end

local function captureV12Part(part)
    if part and not v12OriginalPartState[part] then
        local ok, state = pcall(function()
            return {Size = part.Size, Transparency = part.Transparency, CanCollide = part.CanCollide, Anchored = part.Anchored}
        end)
        if ok then v12OriginalPartState[part] = state end
    end
end
...
Safe.clearErrors = function()
    for index = #errorHistory, 1, -1 do table.remove(errorHistory, index) end
end
...
v13SetBootStage("Environment and Safe layer")
v13BootDiagnostic(v13BootStage, "Core", "environment", "PASS", "", "continue")
...
local function reSurface(object, material, accent)
    ...
    local ok, err = pcall(function()
        ...
        local transparency = material == "Primary" and 0.12 or material == "Secondary" and 0.16 or material == "Floating" and 0.1 or material == "Accent" and 0.08 or 0.2
        ...
        if not gradient then
            ...
            gradient.Name = "REGlassGradient"
            gradient.Rotation = 118
            gradient.Parent = object
        end
        gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(55, 84, 125)),
            ColorSequenceKeypoint.new(0.46, object.BackgroundColor3),
        ...
        if not sheen then
            ...
            sheen.Name = "REGlassSheen"
            sheen.Size = UDim2.new(1, -2, 0, 2)
            sheen.Position = UDim2.new(0, 1, 0, 1)
            sheen.BorderSizePixel = 0
            sheen.BackgroundColor3 = accent or colors.Primary
            sheen.BackgroundTransparency = 0.72
            sheen.ZIndex = object.ZIndex + 1
            sheen.Active = false
...
local function reStyleButton(button, role)
    ...
    local ok, err = pcall(function()
        ...
        button.BackgroundTransparency = role == "Primary" and 0.16 or role == "Danger" and 0.16 or role == "Success" and 0.16 or 0.22
...
local function makeDraggable(topbar, object)
    ...
    rememberV12Connection(topbar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            dragStart = input.Position
            startPos = object.Position

            rememberV12Connection(input.Changed:Connect(function()
    ...
    rememberV12Connection(topbar.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            dragInput = input
        end
    end))

    rememberV12Connection(UserInputService.InputChanged:Connect(function(input)
...
v13SetBootStage("UI creation")
v13BootDiagnostic(v13BootStage, "UI", "ScreenGui", "START", "", "building Console-owned UI")
local previousConsole = rawget(_G, "RE_Ultimate_Console_V13")
if type(previousConsole) == "table" and type(previousConsole.Cleanup) == "function" then
    local previousOk, previousError = pcall(previousConsole.Cleanup)
    if not previousOk then v13BootDiagnostic(v13BootStage, "Core", "previous cleanup", "FAILED", previousError, "continue with owned GUI replacement") end
end
local function destroyOwnedConsoleGui(parent)
    if not parent then return end
    local okOld, oldGui = pcall(function() return parent:FindFirstChild("RE_Ultimate_Framework") end)
    if okOld and oldGui then pcall(function() oldGui:Destroy() end) end
end
if not v13BootstrapGui then
    refreshRuntimeReferences()
    local earlyPlayerGui = nil
    if LocalPlayer then pcall(function() earlyPlayerGui = LocalPlayer:FindFirstChildOfClass("PlayerGui") end) end
    destroyOwnedConsoleGui(Safe.gethui())
    destroyOwnedConsoleGui(earlyPlayerGui)
    destroyOwnedConsoleGui(CoreGui)
end
local gui = v13BootstrapGui or Instance.new("ScreenGui")
gui.Name = "RE_Ultimate_Framework"
gui.ResetOnSpawn = false
gui.IgnoreGuiInset = true
gui.DisplayOrder = 999

local guiParentSet = gui.Parent ~= nil
if not guiParentSet then
    local candidateHosts = {}
    local candidateHui = Safe.gethui()
    if candidateHui then table.insert(candidateHosts, candidateHui) end
    if CoreGui then table.insert(candidateHosts, CoreGui) end
    refreshRuntimeReferences()
    local candidatePlayerGui = nil
    if LocalPlayer then pcall(function() candidatePlayerGui = LocalPlayer:FindFirstChildOfClass("PlayerGui") end) end
    if candidatePlayerGui then table.insert(candidateHosts, candidatePlayerGui) end
    for _, candidateHost in ipairs(candidateHosts) do
        local attached = pcall(function()
            gui.Parent = nil
            gui.Parent = candidateHost
        end)
        if attached and gui.Parent == candidateHost then
            guiParentSet = true
            v13BootstrapHost = candidateHost
            break
        end
    end
end
EnvironmentStatus.UIHost = guiParentSet
v13BootDiagnostic("UI creation", "UI", "ScreenGui", guiParentSet and "PASS" or "FAILED", guiParentSet and "" or "No valid GUI parent", guiParentSet and "continue" or "UI remains unavailable; Core continues")
...
local iconBtn = v13BootstrapIcon or Instance.new("TextButton")
...
iconBtn.Parent = gui
pcall(function() makeDraggable(iconBtn, iconBtn) end)
...
reBuildIcon(iconBtn, 32, true)
reSurface(iconBtn, "Accent", RE_DESIGN.Colors.Accent)
local v13UiReady = false
local v13EmergencyFrame = nil
local v13EmergencyOpenButton = nil
local v13EmergencyStatus = nil

local function v13SetUiStatus(status, detail)
    v13UiReady = status == "READY"
    v13BootDiagnostic(v13BootStage, "UI", "bootstrap", status, detail or "", status == "READY" and "continue optional modules" or "keep emergency UI")
    pcall(function()
        if v13EmergencyStatus then
            v13EmergencyStatus.Text = (status == "READY" and "● UI READY  |  V13" or "○ UI RECOVERY  |  V13") .. (detail and ("\n" .. tostring(detail)) or "")
            v13EmergencyStatus.TextColor3 = status == "READY" and RE_DESIGN.Colors.Success or RE_DESIGN.Colors.Warning
        end
    end)
end

local function v13CreateEmergencyUi(parentGui)
    if not parentGui then return false end
    local ok, err = pcall(function()
        v13EmergencyFrame = Instance.new("Frame")
        v13EmergencyFrame.Name = "RE_V13_EmergencyUI"
        v13EmergencyFrame.Size = UDim2.new(0, 300, 0, 126)
        v13EmergencyFrame.Position = UDim2.new(0.5, -150, 0.5, -63)
        v13EmergencyFrame.BackgroundColor3 = Color3.fromRGB(10, 16, 28)
        v13EmergencyFrame.BorderSizePixel = 0
        v13EmergencyFrame.Visible = false
        v13EmergencyFrame.ZIndex = 900
        v13EmergencyFrame.Parent = parentGui

        local emergencyCorner = Instance.new("UICorner")
        emergencyCorner.CornerRadius = UDim.new(0, 12)
        emergencyCorner.Parent = v13EmergencyFrame
        local emergencyStroke = Instance.new("UIStroke")
        emergencyStroke.Color = RE_DESIGN.Colors.Warning
        emergencyStroke.Thickness = 1
        emergencyStroke.Parent = v13EmergencyFrame

        local emergencyTitle = Instance.new("TextLabel")
        emergencyTitle.Size = UDim2.new(1, -18, 0, 24)
        emergencyTitle.Position = UDim2.new(0, 9, 0, 8)
        emergencyTitle.BackgroundTransparency = 1
        emergencyTitle.Text = RE_DESIGN.Icon .. "  RE ULTIMATE CONSOLE  |  BOOT RECOVERY"
        emergencyTitle.TextColor3 = RE_DESIGN.Colors.Text
        emergencyTitle.Font = Enum.Font.GothamBold
        emergencyTitle.TextSize = 11
        emergencyTitle.TextXAlignment = Enum.TextXAlignment.Left
        emergencyTitle.Parent = v13EmergencyFrame

        v13EmergencyStatus = Instance.new("TextLabel")
        v13EmergencyStatus.Size = UDim2.new(1, -18, 0, 38)
        v13EmergencyStatus.Position = UDim2.new(0, 9, 0, 34)
        v13EmergencyStatus.BackgroundTransparency = 1
        v13EmergencyStatus.Text = "○ UI RECOVERY  |  V13"
        v13EmergencyStatus.TextColor3 = RE_DESIGN.Colors.Warning
        v13EmergencyStatus.Font = Enum.Font.GothamMedium
        v13EmergencyStatus.TextSize = 10
        v13EmergencyStatus.TextWrapped = true
        v13EmergencyStatus.TextXAlignment = Enum.TextXAlignment.Left
        v13EmergencyStatus.Parent = v13EmergencyFrame

        v13EmergencyOpenButton = Instance.new("TextButton")
        v13EmergencyOpenButton.Size = UDim2.new(1, -18, 0, 34)
        v13EmergencyOpenButton.Position = UDim2.new(0, 9, 1, -42)
        v13EmergencyOpenButton.BackgroundColor3 = RE_DESIGN.Colors.Accent
        v13EmergencyOpenButton.BorderSizePixel = 0
        v13EmergencyOpenButton.Text = "فتح Console"
        v13EmergencyOpenButton.TextColor3 = RE_DESIGN.Colors.Text
        v13EmergencyOpenButton.Font = Enum.Font.GothamBold
        v13EmergencyOpenButton.TextSize = 12
        v13EmergencyOpenButton.AutoButtonColor = false
        v13EmergencyOpenButton.Parent = v13EmergencyFrame
        local emergencyButtonCorner = Instance.new("UICorner")
        emergencyButtonCorner.CornerRadius = UDim.new(0, 8)
        emergencyButtonCorner.Parent = v13EmergencyOpenButton

        v13EmergencyOpenButton.MouseButton1Click:Connect(function()
            if menuFrame then menuFrame.Visible = true end
            if inspectorFrame then inspectorFrame.Visible = true end
            if v13EmergencyFrame then v13EmergencyFrame.Visible = false end
        end)
        return true
    end)
    if not ok then
        v13BootDiagnostic(v13BootStage, "UI", "emergency fallback", "FAILED", err, "retain icon only")
        return false
    end
    return true
end

local function v13ValidateIcon(iconObject, parentGui)
    local valid = false
    local ok, value = pcall(function()
        return iconObject ~= nil and parentGui ~= nil and parentGui.Parent ~= nil and iconObject.Parent == parentGui and iconObject.Visible == true
    end)
    valid = ok and value == true
    v13BootDiagnostic(v13BootStage, "UI", "icon validation", valid and "PASS" or "FAILED", valid and "" or "Icon is not visible or parented", valid and "continue main UI" or "show emergency UI")
    return valid
end

local emergencyReady = v13CreateEmergencyUi(gui)
v13BootDiagnostic(v13BootStage, "UI", "ScreenGui", guiParentSet and "PASS" or "FAILED", guiParentSet and "" or "No GUI host", guiParentSet and "icon first" or "icon cannot be visible until host is resolved")
task.spawn(function()
    for attempt = 1, 24 do
        if consoleActive == false or guiParentSet then return end
        refreshRuntimeReferences()
        local playerGui = nil
        if LocalPlayer then pcall(function() playerGui = LocalPlayer:FindFirstChildOfClass("PlayerGui") end) end
        local candidates = {Safe.gethui(), CoreGui, playerGui}
        for _, candidate in ipairs(candidates) do
            if candidate and not guiParentSet then
                local attached = pcall(function()
                    gui.Parent = nil
                    gui.Parent = candidate
                end)
                if attached and gui.Parent == candidate then
                    guiParentSet = true
                    v13BootstrapHost = candidate
                    EnvironmentStatus.UIHost = true
                    v13BootDiagnostic(v13BootStage, "UI", "deferred host retry", "PASS", "", "icon and emergency UI are now visible")
                    local iconReadyAfterAttach = v13ValidateIcon(iconBtn, gui)
                    if iconReadyAfterAttach and not v13UiReady then
                        v13SetUiStatus("READY", "Deferred GUI host attached; bootstrap recovered")
                        if v13EmergencyFrame then v13EmergencyFrame.Visible = false end
                    end
                    if type(v13StartLegacyMonitor) == "function" then pcall(v13StartLegacyMonitor) end
                    break
                end
            end
        end
        if guiParentSet then break end
        task.wait(0.25)
    end
end)
...
task.delay(0.75, function()
    if not consoleActive then return end
    if not v13UiReady and v13EmergencyFrame then
        v13EmergencyFrame.Visible = true
        v13SetUiStatus("RECOVERY", "Main UI did not confirm; emergency open path remains available")
    end
end)
...
title.Text = "  " .. RE_DESIGN.Icon .. "  RE ULTIMATE CONSOLE"
...
pcall(function() makeDraggable(title, menuFrame) end)
...
inspectorTitle.Text = "  " .. RE_DESIGN.Icon .. "  REMOTE INTELLIGENCE  |  V13"
...
pcall(function() makeDraggable(inspectorTitle, inspectorFrame) end)
...
local inspectorStatus = Instance.new("TextLabel")
inspectorStatus.Size = UDim2.new(0, 154, 0, 22)
inspectorStatus.Position = UDim2.new(1, -190, 0, 3)
inspectorStatus.BackgroundTransparency = 1
inspectorStatus.Text = "● ACTIVE   |   V13"
inspectorStatus.TextColor3 = RE_DESIGN.Colors.Success
inspectorStatus.Font = Enum.Font.GothamMedium
inspectorStatus.TextSize = 10
inspectorStatus.TextXAlignment = Enum.TextXAlignment.Right
inspectorStatus.Parent = inspectorFrame

local toolbarFrame = Instance.new("ScrollingFrame")
toolbarFrame.Size = UDim2.new(1, -10, 0, 34)
toolbarFrame.Position = UDim2.new(0, 5, 0, 31)
toolbarFrame.BackgroundTransparency = 1
toolbarFrame.BorderSizePixel = 0
toolbarFrame.ScrollBarThickness = 0
toolbarFrame.ScrollingDirection = Enum.ScrollingDirection.X
toolbarFrame.CanvasSize = UDim2.new(0, 610, 0, 0)
...
modelTitle.Text = "  " .. RE_DESIGN.Icon .. "  إعدادات النموذج الذكي"
...
pcall(function() makeDraggable(modelTitle, modelFrame) end)
...
pcall(function() addGlassStyle(menuFrame, 11, Color3.fromRGB(116, 96, 190), 0.06) end)
pcall(function() addGlassStyle(inspectorFrame, 11, Color3.fromRGB(77, 155, 210), 0.06) end)
pcall(function() addGlassStyle(modelFrame, 10, Color3.fromRGB(116, 96, 190), 0.04) end)
pcall(function() addGlassStyle(aiFrame, 8, Color3.fromRGB(96, 76, 155), 0.08) end)
pcall(function() addGlassStyle(leftScroll, 8, Color3.fromRGB(70, 85, 110), 0.12) end)
pcall(function() addGlassStyle(rightScroll, 8, Color3.fromRGB(70, 85, 110), 0.12) end)
pcall(function() addGlassStyle(aiScroll, 8, Color3.fromRGB(70, 85, 110), 0.12) end)

menuScale = Instance.new("UIScale")
menuScale.Scale = 0.92
menuScale.Parent = menuFrame
inspectorScale = Instance.new("UIScale")
...
styleTopbar(modelTitle, RE_DESIGN.Colors.Secondary)

reSurface(menuFrame, "Primary", RE_DESIGN.Colors.Secondary)
reSurface(inspectorFrame, "Primary", RE_DESIGN.Colors.Primary)
reSurface(inspectorStatus, "Micro", RE_DESIGN.Colors.Success)
reSurface(modelFrame, "Floating", RE_DESIGN.Colors.Secondary)
reSurface(aiFrame, "Secondary", RE_DESIGN.Colors.Secondary)
reSurface(leftScroll, "Secondary", RE_DESIGN.Colors.Border)
reSurface(rightScroll, "Secondary", RE_DESIGN.Colors.Border)
reSurface(aiScroll, "Secondary", RE_DESIGN.Colors.Border)
reStyleButton(toggleSpyBtn, "Primary")
reStyleButton(clearSpyBtn, "Danger")
reStyleButton(copySelectedBtn, "Secondary")
reStyleButton(copyAllBtn, "Secondary")
reStyleButton(dedupBtn, "Success")
reStyleButton(aiTabBtn, "Accent")
reStyleButton(modelTabBtn, "Secondary")
reStyleButton(toggleInspectorBtn, "Primary")
reStyleButton(genBtn, "Accent")
reStyleButton(copyScriptBtn, "Success")
reStyleButton(execAiBtn, "Primary")
reStyleButton(sendEditBtn, "Secondary")
reStyleButton(modelMoreBtn, "Secondary")
reStyleButton(extraModelBtn, "Secondary")
reStyleButton(secondaryModelBtn, "Secondary")
reStyleButton(applyModelBtn, "Success")
reApplyResponsiveLayout()
if Camera then v12UiViewportConnection = rememberV12Connection(Camera:GetPropertyChangedSignal("ViewportSize"):Connect(reApplyResponsiveLayout)) end
local iconReady = v13ValidateIcon(iconBtn, gui)
if iconReady then
    v13SetUiStatus("READY", "ScreenGui + icon + main UI bootstrap complete")
    if v13EmergencyFrame then v13EmergencyFrame.Visible = false end
    v13BootDiagnostic(v13BootStage, "Core", "main UI", "PASS", "", "optional modules may start")
else
    v13SetUiStatus("RECOVERY", "GUI host is unavailable; emergency path remains visible")
    if v13EmergencyFrame then v13EmergencyFrame.Visible = true end
    v13BootDiagnostic(v13BootStage, "Core", "main UI", "FAILED", "Icon host is unavailable", "optional modules remain isolated")
end
...
v13SetBootStage("Legacy UI and handlers")
v13BootDiagnostic(v13BootStage, "Logger", "legacy handlers", "PASS", "", "continue")
...
v12HeartbeatConnection = rememberV12Connection(RunService.Heartbeat:Connect(function()
    if not consoleActive then return end
    if #logQueue > 0 then
...
copySelectedBtn.MouseButton1Click:Connect(function()
    if activeTreeData == "" then
        ...
        task.delay(1.5, function() if not consoleActive then return end; copySelectedBtn.Text = "نسخ المحدد" end)
        return
    end
...
copyAllBtn.MouseButton1Click:Connect(function()
    if #logsDatabase == 0 then
        ...
        task.delay(1.5, function() if not consoleActive then return end; copyAllBtn.Text = "نسخ الكل" end)
        return
    end
...
printSummary must append readable lines to the ScrollingFrame, including timestamp, sender, fullPath, class, method, occurrences, argTypes, and serialized sampleArgs values by looping sampleArgs[i] from 1 to sampleArgs.n. cleanup must stop monitoring, disconnect safely, destroy only this script's ScreenGui, and reset state by assignment. Before emitting, silently verify syntax, all required Arabic labels, packedArgs indexing, pcall boundaries, fixed inbound targets, the original RenderStepped:Connect(function() wrapper, and that the final characters are end). Output code now; output nothing else.]]
    ...
    task.spawn(function()
        if not consoleActive then setAiBusy(false); return end
        ...
        if consoleActive then setAiBusy(false) end
    end)
end
...
local function setupRemoteListener(obj)
    ...
    if not okClass then
        v13BootDiagnostic("Legacy remote monitor", "RemoteMonitor", "setupRemoteListener", "FAILED", isRemote, "skip invalid object")
        return
    end
    ...
    local okConnection, connectionOrError = pcall(function()
        return obj.OnClientEvent:Connect(function(...)
            ...
            task.defer(function()
                ...
                if not okRecord then Safe.reportError("RemoteMonitor", recordError) end
            end)
        end)
    end)
    if okConnection and connectionOrError then
        ...
        v13BootDiagnostic("Legacy remote monitor", "RemoteMonitor", "setupRemoteListener", "FAILED", connectionOrError, "skip listener; Console continues")
    end
end

local function walkV12RemoteListeners(root, depth)
    if not consoleActive or depth > 7 then return end
    local ok, children = pcall(function() return root:GetChildren() end)
    if not ok then return end
    for _, child in ipairs(children) do
        if not consoleActive then return end
        setupRemoteListener(child)
        walkV12RemoteListeners(child, depth + 1)
    end
end

v13StartLegacyMonitor = function()
    if not consoleActive or not guiParentSet then
        v13BootDiagnostic("Legacy remote monitor", "RemoteMonitor", "start", "SKIPPED", "GUI is not ready", "wait for confirmed UI host")
        return false
    end
    walkV12RemoteListeners(game, 0)
    v12DescendantConnection = rememberV12Connection(game.DescendantAdded:Connect(function(obj)
        if not consoleActive then return end
        setupRemoteListener(obj)
    end))
    v13BootDiagnostic("Legacy remote monitor", "RemoteMonitor", "start", "PASS", "", "bounded listener active")
    return true
end
if guiParentSet then v13StartLegacyMonitor() end

v13SetBootStage("Legacy monitor and feature loop")
v13BootDiagnostic(v13BootStage, "RemoteMonitor", "legacy listener setup", "START", "", "bounded discovery")
    ...
    pcall(function()
        ...
        if mt then
            ...
            if rapidEnabled and not Safe.checkcaller() and method == "FireServer" and successRN and remoteNameVal == "RequestActionSync" then
                -- Do not tamper with args or hitInstance; let original server payload pass untouched while maintaining monitoring
            end
...
v12RenderConnection = rememberV12Connection(RunService.RenderStepped:Connect(function()
    if not consoleActive then return end
    refreshRuntimeReferences()
    ...
    if noclipEnabled and char then
        for _, part in ipairs(char:GetChildren()) do if part:IsA("BasePart") then captureV12Part(part); part.CanCollide = false end end
    end

    if invisibleEnabled and char then
        for _, desc in ipairs(char:GetChildren()) do if desc:IsA("BasePart") then captureV12Part(desc); desc.Transparency = 1 end end
    end

    if godModeEnabled and humanoid then
        if not v12OriginalHumanoidState[humanoid] then
            local ok, state = pcall(function() return {MaxHealth = humanoid.MaxHealth, Health = humanoid.Health, DeadEnabled = humanoid:GetStateEnabled(Enum.HumanoidStateType.Dead)} end)
            if ok then v12OriginalHumanoidState[humanoid] = state end
        end
        pcall(function() humanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, false); humanoid.MaxHealth = 9e9; humanoid.Health = 9e9 end)
    end

    if char and char:FindFirstChild("HumanoidRootPart") then captureV12Part(char.HumanoidRootPart); char.HumanoidRootPart.Anchored = ghostEnabled end
...
v13Panel.BackgroundColor3 = Color3.fromRGB(12, 15, 22)

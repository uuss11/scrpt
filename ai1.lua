...
local function validateGeneratedScript(scriptText)
    local required = {
        ...
        "sampleArgs = packedArgs",
        "packedArgs[i]",
        "task.delay(10, printSummary)",
    ...
    local forbidden = {"--!strict", "+=", "-=", "..=", "*=", "/=", "hookmetamethod", "getrawmetatable", "__namecall", "FireServer", "FireClient", "FireAllClients", "InvokeServer", "InvokeClient", "GetDescendants("}
    for _, marker in ipairs(forbidden) do
        if string.find(scriptText, marker, 1, true) then
            return false, "forbidden " .. marker
        end
    end
    local finalText = string.gsub(scriptText, "%s+$", "")
    if not string.match(finalText, "main%(%)[%s]*$") then
        return false, "main() is not the final executable line"
    end
    return true, ""
end
...
local function callOpenRouter(userPromptText)
    ...
    if #chatHistory == 0 then
        table.insert(chatHistory, {
            role = "system",
            content = [[You are a code-only Roblox Luau compiler. Do not explain, reason, plan, summarize, or repeat requirements. Return exactly one complete standalone LocalScript in one ```lua``` code block and nothing outside it. The first response characters must be ```lua. Preserve the complete verified implementation below; do not shorten, replace, or omit functions. If it already satisfies the contract, return it unchanged.

Required contract: plain Luau only; no --!strict, type annotations, generic syntax, continue, compound assignments, table.clear/table.create/table.clone, require, top-level return, executor/server-only APIs, hooks, GetDescendants scanning, FireServer/InvokeServer/FireClient calls, placeholders, TODOs, pseudocode, comments, explanations, or reasoning text. Keep fixed inbound RemoteEvent.OnClientEvent targets, deduplicate by path and method, use sampleArgs = packedArgs with packedArgs.n and packedArgs[i], separate metadata from Instance values, and protect path traversal, IsA, Connect, and Disconnect with pcall. Keep cleanup().

The exact visible UI contract must remain: ScreenGui REZA_DiagnosticUI, OpenButton, MainPanel, UIStroke, UICorner, ScrollingFrame, local appendStatus(text), Arabic title "وحدة تشخيص الريموتات", close text "إغلاق", buttons "بدء المراقبة", "إيقاف المراقبة", "عرض الملخص", "نسخ السجل", "تنظيف السجل", real MouseButton1Click handlers, task.delay(10, printSummary) inside main(), and main() as the final executable line.

<VERIFIED_BASE_IMPLEMENTATION>
local Players = game:GetService("Players")
...
local function serializeValue(value, seen, depth)
    ...
    if valueType == "CFrame" then
        ...
        return string.format("CFrame(%.3f, %.3f, %.3f)", x, y, z)
    end
    if valueType == "Color3" then
        ...
        return string.format("Color3(%.3f, %.3f, %.3f)", r, g, b)
    end
    ...
    if valueType == "Instance" then
        ...
        if userId ~= nil then
            return "Player: " .. tostring(name) .. " [UserId=" .. tostring(userId) .. "]"
        end
        return className .. ": " .. tostring(name)
    end
    if valueType == "table" then
        ...
        for key, item in pairs(value) do
            ...
            table.insert(parts, tostring(key) .. "=" .. serializeValue(item, seen, depth + 1))
        end
        seen[value] = nil
        return "{" .. table.concat(parts, ", ") .. "}"
    end
...
local function getArgTypes(packedArgs)
    local result = {}
    local length = tonumber(packedArgs.n) or 0
    for i = 1, length do
        local item = packedArgs[i]
        ...
        if itemType == "Instance" then
            ...
            itemType = "Instance<" .. tostring(className) .. ">"
        end
...
local function appendStatus(text)
    ...
    if logScroll == nil then
        return
    end
    local entry = Instance.new("TextLabel")
    entry.Name = "LogEntry"
    entry.Size = UDim2.new(1, -8, 0, 22)
    entry.BackgroundTransparency = 1
    entry.Font = Enum.Font.Code
    entry.TextSize = 13
    entry.TextColor3 = Color3.fromRGB(220, 230, 240)
    entry.TextXAlignment = Enum.TextXAlignment.Right
    entry.TextWrapped = true
    entry.Text = tostring(text)
    entry.Parent = logScroll
    pcall(function()
        logScroll.CanvasSize = UDim2.new(0, 0, 0, logScroll.UIListLayout.AbsoluteContentSize.Y + 8)
    end)
end

local targets = {
    {Name = "CharacterMuzzleFlash", Path = "ReplicatedStorage.Events.RemoteEvents.CharacterMuzzleFlash", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "ReplicateFakeBullet", Path = "ReplicatedStorage.Events.RemoteEvents.ReplicateFakeBullet", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "ReplicateSlidingEffect", Path = "ReplicatedStorage.Events.RemoteEvents.ReplicateSlidingEffect", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "ReplicateImpactEffect", Path = "ReplicatedStorage.Events.RemoteEvents.ReplicateImpactEffect", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "RemoteEvent", Path = "ReplicatedStorage.Modules.DataService.Networker._remotes.DataService.RemoteEvent", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "KillfeedEntry", Path = "ReplicatedStorage.Events.RemoteEvents.KillfeedEntry", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "ReplicateJumpPad", Path = "ReplicatedStorage.Events.RemoteEvents.ReplicateJumpPad", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "ShowDeathScreen", Path = "ReplicatedStorage.Events.RemoteEvents.ShowDeathScreen", Method = "OnClientEvent", Direction = "Incoming"},
    {Name = "RagdollRemote", Path = "ReplicatedStorage.Modules.RagdollService.RagdollRemote", Method = "OnClientEvent", Direction = "Incoming"}
}

local function logInbound(meta, remoteInstance, packedArgs)
    local key = meta.Path .. "|" .. meta.Method
    local record = recordByKey[key]
    if record == nil then
        record = {
            ...
            occurrences = 0,
            argTypes = getArgTypes(packedArgs),
            sampleArgs = packedArgs
        }
    ...
    appendStatus(string.format("[%s] %s | تكرار: %d", record.timestamp, record.remoteName, record.occurrences))
end

local function connectTarget(meta)
    local remote = resolvePath(meta.Path)
    if remote == nil then
        return
    end
    local isRemote = false
    pcall(function()
        isRemote = remote:IsA("RemoteEvent")
    end)
    ...
    local ok, connection = pcall(function()
        return remote.OnClientEvent:Connect(function(...)
            local packedArgs = table.pack(...)
            logInbound(meta, remote, packedArgs)
        end)
    end)
...
local function startMonitoring()
    if monitoring then
        appendStatus("المراقبة تعمل مسبقاً")
        return
    end
    monitoring = true
    initialized = true
    for index = 1, #targets do
        connectTarget(targets[index])
    end
    appendStatus("تم بدء المراقبة")
end

local function stopMonitoring()
    if not monitoring then
        appendStatus("المراقبة متوقفة")
        return
    end
    monitoring = false
    ...
    appendStatus("تم إيقاف المراقبة")
end
...
local function buildSummary()
    ...
    local lines = {"=== ملخص المراقبة ==="}
    for index = 1, #records do
        local record = records[index]
        table.insert(lines, string.format("[%s] %s | %s | التكرار: %d | الأنواع: %s", record.timestamp, record.remoteName, record.fullPath, record.occurrences, record.argTypes))
        local length = tonumber(record.sampleArgs.n) or 0
        for i = 1, length do
            table.insert(lines, "  [" .. tostring(i) .. "]: " .. serializeValue(record.sampleArgs[i], {}, 0))
        end
    end
    return table.concat(lines, "\n")
end
...
local function copyLog()
    ...
    if not ok or type(clipboard) ~= "function" then
        appendStatus("الحافظة غير مدعومة")
        return
    end
    local copied = pcall(clipboard, summary)
    if copied then
        appendStatus("تم نسخ السجل")
    else
        appendStatus("فشل نسخ السجل")
    end
end
...
local function addButton(parent, name, text, position, callback)
    ...
    local stroke = Instance.new("UIStroke")
    stroke.Color = Color3.fromRGB(100, 130, 170)
...
local function buildUI()
    ...
    local old = playerGui:FindFirstChild("REZA_DiagnosticUI")
    if old ~= nil then
        old:Destroy()
    end

    screenGui = Instance.new("ScreenGui")
    screenGui.Name = "REZA_DiagnosticUI"
    ...
    openButton.BackgroundColor3 = Color3.fromRGB(50, 35, 90)
    ...
    local iconStroke = Instance.new("UIStroke")
    iconStroke.Color = Color3.fromRGB(170, 120, 255)
    ...
    panelStroke.Color = Color3.fromRGB(110, 130, 180)
    ...
    title.Text = "وحدة تشخيص الريموتات"
    title.Parent = panel

    local closeButton = Instance.new("TextButton")
    closeButton.Name = "CloseButton"
    closeButton.Size = UDim2.new(0, 60, 0, 30)
    closeButton.Position = UDim2.new(1, -68, 0, 10)
    closeButton.BackgroundColor3 = Color3.fromRGB(110, 45, 55)
    ...
    statusLabel.TextColor3 = Color3.fromRGB(170, 220, 180)
    ...
    logScroll.Size = UDim2.new(1, -20, 0, 190)
    logScroll.Position = UDim2.new(0, 10, 0, 210)
    logScroll.BackgroundColor3 = Color3.fromRGB(15, 18, 25)
    ...
    list.Parent = logScroll

    addButton(panel, "StartButton", "بدء المراقبة", UDim2.new(0, 10, 0, 55), startMonitoring)
    addButton(panel, "StopButton", "إيقاف المراقبة", UDim2.new(0, 185, 0, 55), stopMonitoring)
    addButton(panel, "SummaryButton", "عرض الملخص", UDim2.new(0, 10, 0, 95), printSummary)
    addButton(panel, "CopyButton", "نسخ السجل", UDim2.new(0, 185, 0, 95), copyLog)
    addButton(panel, "ClearButton", "تنظيف السجل", UDim2.new(0, 10, 0, 135), clearRecords)
...
Output only the complete code block. Never output the reference tags, analysis, or prose.]]
        })
    end
        ...
        elseif type(decoded) ~= "table" or type(decoded.choices) ~= "table" or not decoded.choices[1] or type(decoded.choices[1].message) ~= "table" then
            appendAiOutput("رد OpenRouter لا يحتوي على choices/message صالح.", Color3.fromRGB(255, 70, 70))
                ...
                if replyContent == "" then
                    ...
                    local scriptMatch = string.match(replyContent, "```lua%s*(.-)%s*```") or string.match(replyContent, "```luau%s*(.-)%s*```") or string.match(replyContent, "```%s*(.-)%s*```") or replyContent
                    ...
                    if not valid then
                        ...
                        if string.find(string.lower(tostring(decoded.choices[1].finish_reason or "")), "length", 1, true) then
                            appendAiOutput("المخرج مبتور بسبب حد التوكنات؛ لم يتم اعتماده.", Color3.fromRGB(255, 120, 70))
                        end
                    else

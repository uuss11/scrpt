...
menuFrame.Size = UDim2.new(0, 330, 0, 325)
...
local stopConsoleBtn = createButton(menuFrame, 8, 286, 314, 30, "إيقاف Console وتنظيفه")
stopConsoleBtn.BackgroundColor3 = Color3.fromRGB(145, 52, 52)
...
local function renderTree(data, parent, indentLevel, textStr)
    for k, v in pairs(data) do
        ...
        row.TextSize = 11
        row.TextColor3 = Color3.fromRGB(220, 225, 235)
...
local function drawLogUI(logData)
    ...
    lbl.TextSize = 10
    lbl.TextColor3 = Color3.fromRGB(235, 240, 248)
...
toggleSpyBtn.MouseButton1Click:Connect(function()
    ...
    if spyRecording and v13StartLegacyMonitor then pcall(v13StartLegacyMonitor) end
...
v13StartLegacyMonitor = function()
    ...
    for _, obj in ipairs(game:GetDescendants()) do
        pcall(function() setupRemoteListener(obj) end)
    end
...
pcall(function()
    ...
    toggleSpyBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
end)
local function v13StopConsoleNow()
    consoleActive = false
    ...
    espEnabled = false
    pcall(function() if type(v13RestoreNoclip) == "function" then v13RestoreNoclip() end end)
    pcall(function()
    ...
    pcall(function()
        for _, connection in ipairs(v12OwnedConnections) do
    ...
    pcall(function()
        local v13 = rawget(_G, "RE_Ultimate_Console_V13")
        if v13 and type(v13.Cleanup) == "function" and not v13Destroyed then v13.Cleanup() end
    end)
    pcall(function() if gui then gui:Destroy() end end)
end
stopConsoleBtn.MouseButton1Click:Connect(v13StopConsoleNow)
...
local function reApplyResponsiveLayout()
    ...
    local ok, err = pcall(function()
        ...
        if menuFrame and menuScale then
            ...
            menuFrame.Size = UDim2.new(0, menuWidth, 0, 325)
...
if guiParentSet then
    v13BootDiagnostic("V13 module initialization", "Core", "startup gate", "READY", "", "manual start on V13 button")
else
    v13BootDiagnostic("V13 module initialization", "Core", "startup gate", "SKIPPED", "GUI host is not ready", "V13 remains idle until the UI host is available")
end

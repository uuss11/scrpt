...
local function v13BootstrapAttach(screenGui)
    ...
    elseif CoreGui then
        table.insert(hosts, CoreGui)
    else
        table.insert(hosts, v13BootstrapPlayerGui())
    end
    for _, host in ipairs(hosts) do
        if host then
            local okParent = pcall(function() screenGui.Parent = host end)
            if okParent and screenGui.Parent ~= nil then
...
if not guiParentSet and CoreGui then
...
task.spawn(function()
    for attempt = 1, 24 do
        ...
        local candidates = {Safe.gethui(), playerGui, CoreGui}
        for _, candidate in ipairs(candidates) do
            if candidate and not guiParentSet then
                local attached = pcall(function() gui.Parent = candidate end)
                if attached and gui.Parent ~= nil then
                    guiParentSet = true
